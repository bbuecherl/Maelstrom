...
var AST = module.exports = {
  generate: function(tokens, filePath) {
    var out = {type: Type.TEMPLATE, childs: AST.createChilds(tokens, filePath)};
    return out;
  },
  ...
  createChilds(tokens, filePath) {
    ...
    for(var i = 0; i < tokens.length; ++i) {
      var ast = AST.create(tokens[i], filePath);
      if(ast)
        childs.push(ast);
    }
    return childs;
  ...
  create: function(token, filePath) {
    var ast = {};
    switch(token.type) {
      case Token.TEXT:
        ast.type = Type.TEXT;
      ...
      case Token.PROCESSOR:
        return Processor.parse(token, ast, AST, filePath);
        break;
      case Token.IGNORED:
        return null;
      default:
